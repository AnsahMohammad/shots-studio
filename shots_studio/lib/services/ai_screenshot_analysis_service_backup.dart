...
class ScreenshotAnalysisService extends AIService {
  ...
  Future<Map<String, dynamic>> _prepareRequestData(
    ...
    if (isLocalModel) {
      // For local model: return prompt text and image file separately
      // Local model can only handle 1 image at a time
      if (images.isEmpty) {
        throw Exception("No images provided for local model analysis");
      }

      final image =
          images.first; // Only process the first image for local model
      File? imageFile;
      ...
      if (image.path != null && image.path!.isNotEmpty) {
        ...
        // Create temporary file from bytes for local model
        final tempDir = Directory.systemTemp;
        final tempFile = File('${tempDir.path}/temp_image_${image.id}.png');
        await tempFile.writeAsBytes(image.bytes!);
        imageFile = tempFile;
      } else {
        throw Exception("No valid image data found for ${image.id}");
      }

      // Return data structure for local model
      return {
        'isLocalModel': true,
        'prompt':
      ...
      // For API: use existing JSON structure
  ...
  Future<Map<String, dynamic>> _makeLocalModelRequest(
    ...
    try {
      ...
      // Get the local model service
      final gemmaService = GemmaService();

      // Ensure model is ready
      await gemmaService.ensureModelReady();

      if (!gemmaService.isModelLoaded) {
        return {'error': 'Local model not available', 'statusCode': 503};
      }

      // Extract prompt and image file
      final String prompt = requestData['prompt'] ?? '';
      final File? imageFile = requestData['imageFile'] as File?;

      if (prompt.isEmpty) {
        return {
          'error': 'No prompt provided for local model',
          'statusCode': 400,
        };
      }

      // Generate response using local model
      final localResponse = await gemmaService.generateResponse(
        prompt: prompt,
        imageFile: imageFile,
        temperature: 0.8,
      );
  ...
  Future<Map<String, dynamic>> _makeAPIRequest(
    ...
    if (requestData['isLocalModel'] == true) {
      return await _makeLocalModelRequest(requestData);
    }
  ...
  Future<AIResult<Map<String, dynamic>>> analyzeScreenshots({
    ...
    final gemmaService = GemmaService();
    await gemmaService.ensureModelReady();
    final isLocalModelAvailable = gemmaService.isModelLoaded;
    final maxParallelCount = isLocalModelAvailable ? 1 : config.maxParallel;

    // Process in batches
    for (int i = 0; i < screenshots.length; i += maxParallelCount) {
      if (isCancelled) {
        finalResults['cancelled'] = true;
        break;
      }

      int end = min(i + maxParallelCount, screenshots.length);
      ...
      try {
        ...
        final requestData = await _prepareRequestData(
          ...
          isLocalModel: isLocalModelAvailable,
        );
